...
class _BookingDetailsState extends State<BookingDetails> {
  ...
  void showConfirmAppointmentBottomSheet({required BookingController controller}){
    ...
    chargeAmount.text = controller.bookingDetailsResponse?.data?.totalAmount ?? "";
                    ...
                    child: Center(
                      child:  Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(controller.businessResponseModel?.data?.contactAddress?.fullAddress ?? "", overflow: TextOverflow.ellipsis,
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        ...
                        controller.bookingDetailsResponse?.data?.notes == null ? const SizedBox():
                        Text(controller.bookingDetailsResponse?.data?.notes ?? "",
                          ...
                          children: [
                            ClipRRect(borderRadius: BorderRadius.circular(50),
                              child: Container(height: 50, width: 50, decoration: BoxDecoration(shape: BoxShape.circle),
                                  child: Image.network(controller.businessResponseModel?.data?.coverImage ?? imagePlaceHolder, fit: BoxFit.cover,)
                              ),
                            ),
                            ...
                            Row(mainAxisAlignment: MainAxisAlignment.start,
                              children: [
                                Column(mainAxisAlignment: MainAxisAlignment.start, crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text(controller.businessResponseModel?.data?.name ?? "",
                                      style: Theme.of(context).textTheme.bodySmall?.copyWith(color: black, fontWeight: FontWeight.w600, fontSize: 16),),
                                    const SizedBox(height: 5,),
                                    Text("₦ ${MoneyFormatter(amount: double.parse(controller.businessResponseModel?.data?.serviceCharge ?? "0.00"),).output.nonSymbol}",
                        ...
                        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                            ...
                            Text("NGN ${MoneyFormatter(
                              amount: double.parse(controller.bookingDetailsResponse?.data?.totalAmount ?? "0.00"),).output.nonSymbol}",
                              style: Theme.of(context).textTheme.bodySmall?.copyWith(color: greenPea, fontWeight: FontWeight.w600, fontSize: 13),),
                          ],
                        ),
                        const SizedBox(height: 5,),
                        _businessController.businessResponseModel?.data?.biography == null ? const SizedBox() :
                        Text(_businessController.businessResponseModel?.data?.biography ?? "",
                  ...
                  DexterPrimaryButton(
                    onTap: (){
                      showMarkDialog(onPressed: (){
                        Get.back();
                        controller.confirmBooking(amount: controller.bookingDetailsResponse!.data!.totalAmount!.toString(), date: dateTimeString.toString(), bookingId: widget.bookingId).then((value){
                          if(value?.statusCode == 200 || value?.statusCode == 201){
                            Get.back();
                            WidgetsBinding.instance.addPostFrameCallback((_) {
                              showDialog(
